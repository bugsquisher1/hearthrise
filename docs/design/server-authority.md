...
4. `supabase/migrations/2026-08-17-market-v2.sql` — **re-derived 2026-08-17.** It SUPERSEDES
   `2026-08-11-market-v2.sql`, which is deleted. The old file was not stale in its detail (all
   seventeen of its review findings are carried forward, by name, in its §0a) but in its **shape**:
   it granted the market RPCs to `authenticated`, its idempotency was weaker than the registry it
   shared a key namespace with, it took no `p_version` and returned no envelope, and it had no
   per-day clamp on the gold flows. Read that file's header before anything else in this section.

Files 3 and 4 **fail closed** if 2 has not been applied. File 4 additionally fails closed without
the *live* `hr_apply` carrying four asserted properties (§0(d), with a positive and a negative scan
control), without `hr_utc_day_key`, and without `hr_assert_grant_hygiene` — its transitive closure
is the whole apply order in `tests/schema-apply-order.json`.
...
Detailed in `supabase/migrations/2026-08-17-market-v2.sql` (the 2026-08-11 file this section used
to name is **deleted** — see the apply list at the top). The headline: once the server owns gold
and inventory, the market gets *simpler*, not more complex.

Three things below were true of the design and NOT of the superseded file, and are now true of
both — they are the reason the file was re-derived rather than patched:

* The three RPCs are **engine-only**. `revoke … from public, anon, authenticated, service_role;
  grant execute … to hr_engine` — the browser reaches them through the Edge verbs in
  `supabase/functions/hr-accrue/market.js` and nowhere else. A browser-callable market RPC is a
  second door into the same tables with its own rate namespace, its own key namespace and no gate.
* Each takes `p_version` and returns `hr_state_of` verbatim, so a client prediction can be
  **retired** — `{ok:true, listing_id}` cannot settle one, and an unsettleable prediction is a
  permanent additive offset rather than a moment of inaccuracy.
* Both sides of every gold flow carry a **per-day clamp** read from the append-only ledger, in
  their own dimension rather than against `hr_day_budget_check` — market gold is conserved, not
  minted, and charging a sale to the seller's daily *mint* budget would let a stranger buy their
  cheap listings and lock them out of their own accrual for the day.
