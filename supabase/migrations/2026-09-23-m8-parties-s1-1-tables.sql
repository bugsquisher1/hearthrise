...
begin
  ...
  raise notice 'm8-parties-s1-1: three tables RLS-armed with SELECT-only policies and no client DML on any of the nine verb/table pairs; both invariants refused their duplicate by unique violation; party_member''s policy resolves through hr_party_of and does NOT recurse (a member reads their roster, a non-member reads zero rows, neither errors); hr_party_view refuses a non-member, answers a member with the FROZEN eight-key shape and NULL settle fields; hr_party_hunt_live is FALSE and S2 owns it; the predicates are callable by no client role and the view by authenticated only, gated; and the block left ZERO rows behind';
end $$;
