// ============================================================================
// supabase/functions/hr-accrue/tick-combat.js — THE COMBAT CHANNEL, end to end.
//
...
// INTENTS — plain objects naming the RPC and its arguments — which `tick.js`
// hands to `hr_tick_settle`.
//
// ── WHERE THIS FILE LIVES, AND WHY IT MOVED HERE (S-8, 2026-09-23) ──────────
// It lived at `services/world-tick/combat.js` until the `op:'tick'` entry
// learned the combat channel. `tools/pack-edge.mjs` walks the static import
// graph from `index.ts` and can only vendor from `src/core`, `src/data` and the
// function directory — so a module under `services/` is UNREACHABLE from an
// edge payload. While the settler sat there, `tick.js` fenced every character
// as 'gather', `settleCombatSession` had no production caller, and the 48 h
// combat parity read was structurally empty: Security's S-8, filed against the
// head of 2026-09-22 and a P0 for the arm.
//
// So it MOVED, and `services/world-tick/combat.js` is now a re-export of this
// file — exactly as gather did on 2026-09-21. That is a MOVE, never a copy:
// there was one copy before and there is one copy after. A second combat path
// is what AWAY-12 forbids, and the drift would be SILENT — the offline guards
// would keep measuring a file the deployed function no longer runs. The fixture
// half (`loadCombatSessions`, `atSpan`) stays behind because it reads
// `node:fs`, which does not belong in a payload.
//
...
/* THE IDEMPOTENCY KEY, IMPORTED AND NOT RE-SPELLED. `tickIntentId` is
   ...
   already. One copy means a channel cannot drift into a weaker key. */
import { tickIntentId } from './tick-gather.js';
