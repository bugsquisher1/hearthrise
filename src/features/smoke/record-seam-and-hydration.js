...
// ══════════════════════════════════════════════════════════════════════
import { pass, fail, tryRun, tryRunAsync, assert, skip, stampBalanceLikeLoad, stampRecordLikeLoad, withLocalBlob, applyAwayEnvelope, predZero, snapshotG, armActivityTransport, drain, restoreAccrualSwitch, seedPlayStreak, restoreG, restoreGAndRecord, on, snapshot, freshFrameGate, closeOverlays } from './_harness.js?v=553';
...
export default [
  ...
  () => tryRunAsync('B338-1: the SERVER\'s starting kit still equals the CLIENT\'s fresh character', async () => {
    ...
    const KIT = await import('../../data/start-kit.js?v=553');
  ...
  () => tryRun('B339-5: the server character REPLACES local progress, and it says so before it does it', () => {
    ...
    try {
      ...
      /* THREE PROBE CHARACTERS, ONE FIXTURE ENVELOPE (M5). `veteran()`, `G2` and
         ...
         whose floor it is starting from. See freshFrameGate's own note. */
      freshFrameGate();
  ...
  () => tryRunAsync('B492-3b: the capstone boot branch actually CALLS the forget, and the record read is not hostage to the ensure', async () => {
    ...
    const src = await (await fetch('src/legacy.js?v=553')).text();
...
function m5FrameGateTests() { return [
  ...
  () => tryRun('M5 regression: ONE frame floor, shared by all three appliers', () => {
    ...
    try {
      ...
      assert(stale && stale.stale === true && stale.verdict === 'reorder',
