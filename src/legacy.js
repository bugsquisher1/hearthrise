...
function startSkill(type,targetId,ms){
  ...
  const actualMs=Math.max(500,Math.floor(pacedActionMs(ms)*speedClamp(speed)));
...
window.startArtisan = function(skillId, recipeId){
  ...
  var actualMs = Math.max(500, Math.floor(window.pacedActionMs(r.ms) * speedClamp(speed)));
  G.skillMs = actualMs;
