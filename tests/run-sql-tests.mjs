...
const ALSO_LINTED = [
  ...
  /* Link 7 of HR_GRANT_HYGIENE_CHAIN — the attended kill ledger. It creates the
     SECURITY DEFINER read projection hr_attended_kills (revoked from every client
     role, granted to hr_engine only) AND `create or replace`s THE DETECTOR to
     record that one grant, which the nightly run raises on until it is
     allowlisted. GENERATED by tools/derive-grant-hygiene.mjs (LINKS[6]) from
     link 6's body, insertions only. Listed here so the grant lints see it AND
     PART 1f-ii can walk its derivation; it is the NEW last toucher of
  ...
  /* NON-SHOP companion grant. Creates a client-callable (authenticated) SECURITY
     DEFINER RPC hr_companion_grant that writes a companion:<id> ownership row for a
     legitimate non-shop acquisition, plus the allowlist catalogue hr_companion_grants
     it gates on. Listed here so the grant lints see the create + revoke/grant block,
     PART 1b-ii (A9) confirms it references a rate gate (hr_rate_ok), and it is checked
     against CLIENT_CALLABLE. It touches none of the graded bodies (hr_apply/
     hr_state_of/hr_perks_of/hr_rate_gate/hr_assert_grant_hygiene), so it is on no
     derivation chain. GENERATED by tools/gen-companion-grants.mjs from
     src/data/companions.js (drift-guarded; run-smoke preflight). */
  '2026-08-22-companion-grant.sql',
  /* P0 ACCESS CONTROL — the closed-beta invite gate (2026-08-23). Creates five
     functions in `public`, NONE of them client-callable: hr_beta_code_norm,
     hr_beta_gate_reason, hr_beta_gate_message, hr_signup_gate (granted only to
     supabase_auth_admin, as the before-user-created auth hook) and the two
     trigger bodies hr_beta_gate_on_auth_user / hr_beta_signup_log_immutable.
     Listed here because that is exactly the case this list exists for: a new
     SECURITY DEFINER function is born PUBLIC=EXECUTE unless the file revokes,
  ...
  /* §1 of the dungeon settlement — Dungeon Scrip becomes server-of-record. It
     PATCHES hr_state_of programmatically (pg_get_functiondef + a guarded
     exactly-once anchor, no literal create-or-replace header, no last-toucher
     role) to project player_state.dungeon_scrip beside marks, and restates the
     hr_state_of revoke/grant explicitly. On no derivation chain. */
  '2026-09-10-dungeon-scrip.sql',
  /* §2 of the dungeon settlement — hr_dungeon_settle. It creates an ENGINE-ONLY
     SECURITY DEFINER writer of player_state.dungeon_scrip / player_inventory and
     `create or replace`s THE DETECTOR (hr_assert_grant_hygiene) to admit the one
     new engine grant, so it is the CURRENT last toucher of the detector; PART
     1f-ii pins that chain as its EIGHTH link. ⚠ REBASED for b504 (increment 2 cut
     it as link 7 on live-progress; b504's attended kill ledger is now link 7, so
     this chains onto 2026-09-10-attended-loot-credit.sql). GENERATED by
     tools/derive-grant-hygiene.mjs (LINKS[7]) from attended-loot-credit's body,
     insertions only. Listed here so `sources` holds it for the derivation walk
     and the grant lints see its create-or-replaces + the new engine grant. */
  '2026-09-10-dungeon-settle.sql',
  /* §4 of the dungeon settlement (increment 3) — hr_quartermaster_buy. It creates
     an ENGINE-ONLY SECURITY DEFINER writer of the scrip SPEND (debits
     player_state.dungeon_scrip, credits player_inventory) and `create or replace`s
     THE DETECTOR to admit the one new engine grant, so it is the CURRENT last
     toucher of the detector; PART 1f-ii pins that chain as its NINTH link.
     GENERATED by tools/derive-grant-hygiene.mjs (LINKS[8]) from dungeon-settle's
     body, insertions only. Listed here so `sources` holds it for the derivation
     walk and the grant lints see its create-or-replace + the new engine grant. */
  '2026-09-11-quartermaster-buy.sql',
  /* The world tick's settle fence (set/b551, APPLIED 2026-09-22 21:26 UTC). It
     `create or replace`s THE DETECTOR to admit hr_tick_settle, so it is a link
     of the derivation chain — tools/derive-grant-hygiene.mjs LINKS[9]. It was
     not on this list when b551 wrote it, which left PART 1f-ii walking straight
     from quartermaster-buy to the next restatement and therefore unable to see
     this body at all; the walk cannot grade a file `sources` does not hold. */
  '2026-09-21-engine-allowlist-tick-settle.sql',
  /* M7, the Bestiary trophy ladder (docs/design/BESTIARY_LADDER.md §4). It creates
     an ENGINE-ONLY SECURITY DEFINER read (hr_trophy_of) and an ENGINE-ONLY writer
     (hr_trophy_claim, which writes one collection row and one ZERO-valued ledger
     row and mints nothing), and `create or replace`s THE DETECTOR to admit those
     two engine grants — so it is the CURRENT last toucher of the detector; PART
     1f-ii pins that chain as its ELEVENTH link. ⚠ REBASED 2026-09-22 on the
     merge of set/b551: cut as the tenth link on quartermaster-buy, it is now
     GENERATED by tools/derive-grant-hygiene.mjs (LINKS[10]) from the settle
     fence's NOW-LIVE body, because an older base would silently revert the
     hr_tick_settle grant. Insertions only. Listed here so `sources` holds it for
     the derivation walk and the grant lints see its create-or-replaces + the two
     new engine grants. */
  '2026-09-22-trophy-claim.sql',
  /* Link 13 of HR_GRANT_HYGIENE_CHAIN — the M3 shadow-state chain. It
     `create or replace`s THE DETECTOR to restate hr_tick_settle's allowlist
     entry at its new ten-argument signature, and DROPS the nine-argument
     overload — which is why this link is a REPLACEMENT and not an insertion.
     GENERATED by tools/derive-grant-hygiene.mjs (LINKS[12]). Listed here so the
     grant lints see §3's re-issued revoke-then-grant (a dropped function takes
     its ACL with it, so those lines are load-bearing, not belt-and-braces) AND
     so PART 1f-ii can walk its derivation. It is the CURRENT last toucher of
     hr_assert_grant_hygiene. */
  '2026-09-23-world-tick-shadow-state-chain.sql',
  /* M8 SLICE 2 — the three party files. All three create functions in `public`
     (hr_partied, hr_party_hunt_live's real body, hr_party_roster,
     hr_party_tick_settle) and all three carry revoke-then-grant lines, so the
     grant lints below are exactly the defence they need; the third is
     GENERATED by tools/derive-grant-hygiene.mjs (LINKS[13]) and is listed so
     PART 1f-ii can walk its derivation. It is the CURRENT last toucher of
     hr_assert_grant_hygiene. */
  '2026-09-23-m8-parties-s2-1-hunt-tables.sql',
  '2026-09-23-m8-parties-s2-2-roster-settle.sql',
  '2026-09-23-m8-parties-s2-3-engine-allowlist.sql',
  /* SLICE 7 - the hr_apply restatement. Listed here so sources holds it for the
     derivation walk (it is HR_APPLY_CHAIN's last link) and so the grant lints
     see its create-or-replace and its revoke-then-grant. */
  '2026-09-14-hr-apply-restatement.sql',
  /* SLICE 7 - the hr_state_of restatement (chain depth 24 -> 0). Listed here so
     `sources` holds it for the derivation walk (it is HR_STATE_OF_CHAIN's last
     link, and until it was listed the last-toucher check could not even SEE it:
     tests/live-settlement.mjs SETTLE-SQL has been red on main since b2eb9943
...
const HR_APPLY_CHAIN = [
  ...
  '2026-08-25-workers.sql',
  /* SLICE 7 — THE HONEST RESTATEMENT. Not derived from ONE predecessor by one
     tools/derive-*.mjs, which is what every link above is: it is the chain-end
     body READ BACK with pg_get_functiondef after the repo chain was replayed,
     i.e. workers.sql plus the 44 anchored patches thirteen later files spliced
     into it. That is why it is the last link and why its declared removals are
     the lines those 44 patches REPLACED. */
  '2026-09-14-hr-apply-restatement.sql',
];
...
const HR_STATE_OF_CHAIN = [
  ...
  '2026-08-26-marks-record.sql',
  // SLICE 7, THE HONEST RESTATEMENT. hr_state_of's chain depth was 24 anchored
  // patches across 23 files since marks-record; this file is pg_get_functiondef
  // taken AT CHAIN END (the whole repo chain replayed, the installed text read
  // back) with comment-only cleanup, so it restates the WHOLE body and takes the
  // depth to 0. It is the new last toucher of hr_state_of. Its three declared
  // removals are below and every one is an object terminator that GAINED a key —
  // in particular the `fight` projection, the streak fields, the crew array and
  // the FULL waterings array all survive verbatim, which is the property this
...
const HR_GRANT_HYGIENE_CHAIN = [
  ...
  '2026-08-20-live-progress-engine-allow.sql',
  // link 7 — the attended kill ledger. An INSERTION (one engine grant at the head
  // of c_engine_allow: hr_attended_kills, `stable sql`, one character, granted by
  // 2026-09-10-attended-loot-credit.sql). The settle is the ONE writer of loot and
  // gold and it priced attended windows as unattended — measured 9 kills against
  // 15 the server had already accepted and clamped. Insertions only, so its
  // declared-removals list below stays EMPTY. It is the new last toucher.
  '2026-09-10-attended-loot-credit.sql',
  // link 8 — the dungeon settle writer. ⚠ REBASED for b504: increment 2 cut this
  // as link 7 on base 2026-08-20-live-progress-engine-allow.sql; b504 landed the
  // attended kill ledger as link 7, so this now chains onto it (its base in
  // tools/derive-grant-hygiene.mjs LINKS[7] is 2026-09-10-attended-loot-credit.sql)
  // — an earlier base would silently revert link 7's hr_attended_kills grant. An
  // INSERTION (one engine grant at the head of c_engine_allow: hr_dungeon_settle,
  // granted to hr_engine by 2026-09-10-dungeon-settle.sql, which the nightly
  // detector's check (7) would otherwise raise engine_execute_outside_allowlist
  // on). Insertions only, so its declared-removals list below stays EMPTY. It is
  // the new last toucher.
  '2026-09-10-dungeon-settle.sql',
  // link 9 — the Quartermaster spend writer. Chains onto link 8's NOW-LIVE body
  // (dungeon-settle; its base in tools/derive-grant-hygiene.mjs LINKS[8] is
  // 2026-09-10-dungeon-settle.sql), so it cannot silently revert link 7's
  // hr_attended_kills or link 8's hr_dungeon_settle grant. An INSERTION (one
  // engine grant at the head of c_engine_allow: hr_quartermaster_buy, engine-only
  // like hr_dungeon_settle, granted to hr_engine by 2026-09-11-quartermaster-buy.sql,
  // which the nightly detector's check (7) would otherwise raise
  // engine_execute_outside_allowlist on). Insertions only, so its declared-removals
  // list below stays EMPTY. It is the new last toucher.
  '2026-09-11-quartermaster-buy.sql',
  // link 10 — the world tick's settle fence (set/b551, APPLIED 2026-09-22 21:26
  // UTC). An INSERTION (one engine grant at the head of c_engine_allow:
  // hr_tick_settle, granted to hr_engine by 2026-09-21-world-tick-settle-fence
  // .sql). Insertions only, so its declared-removals list below stays EMPTY.
  '2026-09-21-engine-allowlist-tick-settle.sql',
  // link 11 — the Bestiary trophy pair (docs/design/BESTIARY_LADDER.md §4).
  // ⚠ REBASED 2026-09-22 on the merge of set/b551: cut as link 10 on base
  // 2026-09-11-quartermaster-buy.sql, it now chains onto link 10's NOW-LIVE body
  // (its base in tools/derive-grant-hygiene.mjs LINKS[10] is
  // 2026-09-21-engine-allowlist-tick-settle.sql), so it cannot silently revert
  // link 10's hr_tick_settle grant or links 7-9. An INSERTION (TWO engine grants at
  // the head of c_engine_allow: hr_trophy_of, a STABLE per-character read that is
  // now the ONLY door to the trophy population — 2026-09-22-state-of-trophy-prefix
  // .sql took it out of hr_state_of's 1000-row envelope — and hr_trophy_claim, a
  // writer whose whole caller-supplied surface is a slot, a catalogued monster id,
  // a bounded stage and an idem uuid, with NO kill-count parameter and NOTHING
  // minted). Both are granted to hr_engine by that file, which the nightly
  // detector's check (7) would otherwise raise engine_execute_outside_allowlist on
  // — and did, in the in-page suite, before this link was written. Insertions
  // only, so its declared-removals list below stays EMPTY. It is the new last
  // toucher.
  '2026-09-22-trophy-claim.sql',
  // link 13 — the M3 shadow-state chain, and THE FIRST REPLACING LINK of this
  // chain. 2026-09-23-world-tick-shadow-state-chain.sql gives hr_tick_settle a
  // tenth parameter (`p_shadow_state jsonb default null`) so a SHADOW window is
  // seeded from where the last one left the character instead of from the frozen
  // player_state row, and DROPS the nine-argument overload. c_engine_allow is
  // keyed on `regprocedure`, so the changed signature is a DIFFERENT STRING: the
  // old line is REMOVED (declared below — this chain's first non-empty list) and
  // the ten-argument one inserted in its place. Leaving the old line would record
  // a door that no longer exists while the real one raises
  // `engine_execute_outside_allowlist` on the nightly cron every night.
  //
  // ⚠ ITS BASE IN tools/derive-grant-hygiene.mjs IS LINKS[11]'s TARGET,
  //   2026-09-22-engine-allowlist-hunt-reads.sql, WHICH THIS FILE DOES NOT NAME
  //   AT ALL — not in this chain and not in ALSO_LINTED. Pre-existing and not
  //   introduced here: link 12 applied to production on 2026-09-23 06:25 UTC and
  //   was never registered with these lints. The declaration below is correct
  //   either way, because hunt-reads is insertions-only and the only line this
  //   file removes from EITHER predecessor's body is the nine-argument signature
  //   — but the walk grades a two-link jump and link 12's own body is ungraded.
  //   Reported to the Coordinator rather than fixed here: it is a one-line
  //   change in somebody else's lane, and making it silently in this one would
  //   hide how long the hunt link has gone unlinted.
  '2026-09-23-world-tick-shadow-state-chain.sql',
  // link 14 (M8 S2 parties) — the party fence and invariant 7's predicate. An
  // INSERTION (two engine grants at the head of c_engine_allow:
  // hr_party_tick_settle, granted by 2026-09-23-m8-parties-s2-2-roster-settle
  // .sql, and hr_partied, granted by ...-s2-1-hunt-tables.sql), so its
  // declared-removals list below stays EMPTY. Its base is link 13's TARGET —
  // the body production has run since 20:06 UTC — so it cannot revert the
  // ten-argument hr_tick_settle signature that link recorded; the file's own
  // §1 reads the INSTALLED detector for that signature and refuses the apply
  // if it is not there. It is the new last toucher.
  '2026-09-23-m8-parties-s2-3-engine-allowlist.sql',
];
...
const CLIENT_CALLABLE = new Map([
  ...
  /* The NON-SHOP companion grant — writes a companion:<id> ownership row for a
     legitimate drop/quest/hatch/skill/boss acquisition so it survives the capstone
     arm's reconcileCompanions rebuild. Allowlist-gated (hr_companion_grants — a
     shop or forged id is refused 'not_grantable'), owned-once, rate-gated
     (companion_grant bucket via hr_rate_ok), deliberately NOT granted to hr_engine
     (2026-08-22-companion-grant.sql §4 records the hr_client_rpc_baseline row). */
  ['hr_companion_grant', ['authenticated']],
...
say(`── ${SPEC.fn} derivation chain (each body derived from the last, nothing retyped)`);
{
  ...
  const DECLARED_REMOVALS = {
    /* hr_assert_grant_hygiene: 2026-09-23-world-tick-shadow-state-chain.sql is
       THE FIRST LINK OF THAT CHAIN THAT REPLACES AN ENGINE ENTRY. It carries
       its predecessor's body verbatim — every check and every other allowlist
       entry — and changes exactly ONE line: hr_tick_settle's signature, which
       gained a tenth parameter (`p_shadow_state jsonb`) and is therefore a
       different `regprocedure` string. The removal is the OLD nine-argument
       line, and the migration DROPS that overload in the same file — so leaving
       the line here would record a door that does not exist while the real one
       raises `engine_execute_outside_allowlist` nightly. Anything else it
       dropped would be undeclared and fail. */
    '2026-09-23-world-tick-shadow-state-chain.sql': [
    ...
    /* ── SLICE 7, THE HONEST RESTATEMENT. Not one derivation but forty-four:
       this body is workers.sql's PLUS every anchored patch thirteen later files
       spliced into it, read back with pg_get_functiondef at chain end. The
       remarkable — and load-bearing — result is that only TWO of workers.sql's
       lines did not survive, and both are ARRAY TERMINATORS that later files
       extended rather than rules anybody dropped:
         · c_delta_keys' terminator, which gained 'rested_xp'/'rested_at' and
           then eleven more keys (buff_apply, hearthfind, consec_falls,
           last_away_receipt, recovering_until, deaths, …).
         · c_release_codes' 'version_conflict' line, which was reflowed when the
           list grew past one line.
       A THIRD removal here would mean the restatement lost a rule, which is the
       one thing a restatement must never do and the reason this chain grades it
       at all. The migration's own §3(a) is the behavioural half of the same
       question, with one derived section-unique marker per section. */
    '2026-09-14-hr-apply-restatement.sql': [
  ...
  const DECLARED_REMOVALS_BY_FN = {
    hr_state_of: {
      ...
      '2026-08-26-marks-record.sql': [],
      /* ── SLICE 7, THE HONEST RESTATEMENT. Not one derivation but twenty-four:
         this body is marks-record's PLUS every anchored patch twenty-three later
         files spliced into it, read back with pg_get_functiondef at chain end
         (144 lines -> 782). THREE of marks-record's lines did not survive, and
         all three are jsonb_build_object TERMINATORS that later files EXTENDED —
         not one rule anybody dropped:
           · the `streak_day_key` terminator, which gained combat_style and the
             thirteen state keys after it (`),` -> `,`);
           · the worker object's `xp`/`acc_ms` terminator, which gained
             `hired_at` (2026-09-12-worker-hired-at-projection.sql, the 24h
             first-hire backlog fix);
           · the farm plot's `planted_at`/`watered_at` terminator, which gained
             the FULL `waterings` array (Q-5, 2026-09-06-state-of-farm-projection).

         ⚠ A FOURTH REMOVAL HERE MEANS THE RESTATEMENT LOST A PROJECTION, and the
           one this chain was built for is `fight`: the accrual window carries the
           in-progress monster's HP in state.fight, so a restatement that dropped
           it would start every window with a fresh monster at full HP and no
           self-check in the file would notice. `'fight', v_st.fight,` and the
           streak / crew / waterings / inventory_complete lines are carried
           verbatim and are therefore correctly NOT listed. */
      '2026-09-14-hr-state-of-restatement.sql': [
...
say('── every currency delta key has a per-call clamp AND a daily ceiling (Security 2026-08-15)');
{
  ...
  const DELTA_KEY_CLASS = {
    ...
    /* ── THE KEYS THE RESTATEMENT BRINGS ONTO THIS LIST. Every one was already
       live; none had ever reached this classification, because the chain's last
       link was workers.sql and these arrived by anchored patch. Each is stated
       here because the rule is that somebody decides IN WRITING whether a key
       moves value — not that the key is new. */
    buff_apply: ['pointer', 'names ONE item; the type, magnitude, duration and expiry are resolved '
      + 'server-side from hr_item_buffs and now(). An object carrying any other key is refused '
      + 'bad_buff_shape, and the buff must be PAID for in the same delta (buff_not_paid)'],
    hearthfind: ['pointer', 'names an item; the trophy, its legitimacy, the pairing, the day count '
      + 'and the instant are all re-derived from hr_hearthfind_items/_sources. At most one per '
      + 'apply, three per character per UTC day, and it cannot be minted through `items`'],
    consec_falls: ['bounded', 'Recovery rev. 3 — an ABSOLUTE non-negative integer counting falls with '
      + 'no kill between. Engine output; gates nothing a player wants and enters no conservation sum, '
      + 'so it is checked for shape and blast radius only'],
    last_away_receipt: ['metadata', 'the away card. It DESCRIBES the value the rest of the same delta '
      + 'moved and moves none itself; every number in it is re-derived against the locked row and '
      + 'now(), and a sync-sized span is refused rather than stored'],
    recovering_until: ['bounded', 'First-Night Idle Rescue — an ABSOLUTE instant, ceilinged ahead of '
      + 'now() and REFUSED when it would shorten a running knockout (Security F1). A clock, never a '
      + 'balance'],
    deaths: ['metadata', 'rev. 2 N3 — an array of server-derived death records fanned out to one '
      + 'ledger row each. Audit only: the rows carry no gold_in/xp_in/qty_in, so they enter no '
      + 'conservation sum and there is nothing here to clamp, only to refuse'],
    rested_xp: ['bounded', 'b437 — the ABSOLUTE rested bank. Engine output, clamped at the write; it '
      + 'is a MULTIPLIER budget, not a balance, and it is not tradeable or rankable'],
    rested_at: ['clock', 'the rested bank\'s own watermark, advanced from now() by the server'],
