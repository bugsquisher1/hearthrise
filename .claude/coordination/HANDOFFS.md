...
mechanical: the two coordination files (union, main's entries first) and ONE import line in
`src/net/record.js` (union of names at `?v=519` — one line, never two). Everything else
auto-merged; I verified each auto-merge by reading both sides rather than trusting git.

**`tests/ci-shape.baseline.json` was regenerated by its own tool** (`node tests/ci-shape.mjs
--write`), never hand-merged. The delta vs main is exactly my two `away-receipt-journal` commands.
Both lanes' workflow steps survived in their own jobs (mine in `economy-selftests`, main's
`token-single-source` in `client-guards`); the 5-job matrix shape is intact — 86 commands, 86
distinct.

**⚠ THE ONE THING TO KNOW.** b519's away-card holder and this branch's restore are individually
correct and together reopened b519's own bug — see CONFLICTS.md for the measurement and the
one-line resolution in `reconcileAwayReceipt`. **New in-page test `F1b`** pins it, graded on the
rendered band through the real path, mutation-proved red. If you bisect this branch, F1b is the
test that only passes on the MERGED tree — it cannot pass on either parent.

**Verified (no full suite, no run-ci-local — memory pressure, as briefed):**
`bump-version.sh --check` (all at 519) · `ci-shape` + `--selftest` (8/8 caught, 2 controls silent) ·
`away-receipt-journal` base + `--selftest` (14/14 mutations caught) · `schema-drift` (byte-identical
second apply) · `dead-exports` (1209 names, 0 dead) · `window-globals-exist` · filtered in-page run
in ONE page: **29/29 green** across `F1`, `F1b`, `SYNC-` (10), `RECOVER-` (11), `B348` (6), **0
console errors**. `pack-edge hr-accrue --hash` is UNCHANGED at
`16c371f5629b1b83bb48d2934f41d69723130f086dae779d60c2dc389411e9e3` — main did not touch
`supabase/functions/**`, so **no edge redeploy is owed by this merge**.

**Still owed by the ORIGINAL branch, unchanged by this merge:** the away receipt's
`stoppedBy`/`deaths`/`recoverMs` fields are produced by the engine and dropped at the response
boundary (`hr-accrue/index.ts ~1163`), so b519's new toast clauses and the card's stop copy read
null on a LIVE envelope. Filed already; naming it again because b519 shipped the renderers for
...
`art-direction.css`, `board-and-shop.css`, `index.html`, `tests/token-single-source.mjs` (new
guard) + baseline, `tests/breakpoints.json`, `.github/workflows/smoke.yml`, `ci-shape.baseline.json`,
`tools/css-ab-pixel-diff.mjs` (new). Commits `5af5bb36`, `7e9303b2`. Not bumped, not pushed.

**What you need to change in your habits.** A design token is declared in `src/styles/tokens.css`
and NOWHERE else. Adding `:root{--new-thing:...}` to the bottom of the sheet you happen to be in is
now a red build with your file and line. If the value varies per ELEMENT rather than per theme it
is a component-scoped variable: add it to `COMPONENT_SCOPED` in the guard with a reason. Do not
reorder blocks inside tokens.css to tidy them — the file reproduces the old cascade by being in the
old order, and reordering it is a visual change.

**Use the tool before you claim a refactor changed nothing.**
`node tools/css-ab-pixel-diff.mjs` diffs the working tree against HEAD at 1440x900 and 922x423 on
combat/inventory/home/farm; `--control` proves the noise floor is 0 on your machine; `--mutate`
proves it can still see a change. Run `--control` alongside any zero you intend to report. Two
separate browser runs are NOT comparable (measured: 38,799 px of pure noise on an unchanged tree);
the tool exists because that number made "looks the same to me" worthless.

**One line of CLAUDE.md is now stale and I did not edit it myself.** §7 says "Every colour is a
CSS token from `theme-cozy.css`"; it is `tokens.css` as of `5af5bb36`. That is the instruction
file rather than a coordination doc, so it is the Coordinator's edit to make. §7's mobile-
breakpoint paragraph is still correct: converging the eight 900px rules remains open as slice 5b.

**For the Coordinator, at integration:** this branch is CSS + tests + tools only. No `src/**/*.js`,
no `supabase/**`, no migration, no bump. It conflicts with any branch that edits a `:root` block in
one of the four sheets — resolve those as "the declaration belongs in tokens.css", never by
re-adding the root block.
