...
replay in `tests/ledger-rollup.mjs`. **Doc size note: this file is now ~490
lines and is still one readable document; the next section should split it by
SUBJECT (not by date), not start a new file.**
