...
const TESTS = [
  ...
  () => tryRun('b228: the save migration reserves the Chronicle without inventing history', () => {
    const M = (window.HEARTHRISE_MIGRATIONS || []).find((m) => m.from === 8 && m.to === 9);
    assert(M, 'the v8 → v9 Chronicle migration is missing');
    assert(window.HEARTHRISE_SCHEMA_VERSION >= 9, 'CURRENT_SCHEMA_VERSION was not bumped to 9');
    const save = { v: 8, skills: { woodcutting: 999999 } };
    M.apply(save);
