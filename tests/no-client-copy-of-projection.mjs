...
//
// That replay costs ~10 s and a pglite, which is the db-replay-3 job's budget
// (split from db-replay 2026-09-20, then from db-replay-2 2026-09-26) and not
// the client-guards / lane-done budget. So the executed answer is PINNED in
// tests/no-client-copy-of-projection.baseline.json:
//   · `--execute`         replays and FAILS if the pinned set differs from the
//                         real one (registered in the db-replay-3 job).
//   · the default run     is pure text over src/ + the pinned set, so every lane
//                         hits it in `tools/lane-done.mjs` for ~1 s.
// A migration that adds a projection therefore goes red in db-replay-3 until
