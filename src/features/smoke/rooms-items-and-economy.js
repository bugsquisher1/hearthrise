...
// ══════════════════════════════════════════════════════════════════════
import { pass, fail, tryRun, tryRunAsync, assert, skip, bountyRig, stampBalanceLikeLoad, stampRecordLikeLoad, withServerBacked, withRoomServer, awayArtisanSpan, tryRunRestampingBalance, goldOf, gemsOf, snapshotG, setAway, drain, restoreG, restoreGAndRecord, restoreBankCap, on, snapshot } from './_harness.js?v=553';
...
export default [
  ...
  () => tryRunAsync('b505: the store sells no gold and no accrual boost — permanently', async () => {
    ...
    const S = await import('../../data/shops.js?v=553');
  ...
  () => tryRunAsync('DGN-SETTLE-1: src/data/dungeons.js matches the client window.DUNGEONS (server catalogue = render source)', async () => {
    const D = window.DUNGEONS;
    if (!D) return;
    const mod = await import('../../data/dungeons.js?v=553');
