...
| **Live gather/craft/combat-drop → server (Model A)** — THE SINGLE ACTIVE TRACK | 🔧 | **P0** | Now the sole priority (Tyler, 2026-08-17: server-authority first, no parallel feature work). **Step 1 IN PROGRESS**: systems-engineer auditing whether the settle loop runs live + which live bag-writers are un-modelled (esp. crops + cooking) — the "confirm first" gate. Then step 2: convert kill/gather/craft client writers to server-settled predictions (UNARMED, watch `envelopeDrift`) → restore drill → wipe → arm. ~1 week; foundation largely built. |
