...
| **Inventory flip → arm "server owns the bag"** — THE SINGLE ACTIVE TRACK | 🔧 | **P0** | Sole priority (Tyler, 2026-08-17). **Step 1 DONE (audit):** the live settle loop already server-settles combat drops, gathering, non-cooking crafting, market, auto-eat (MODELED). UN-MODELED (would be deleted if armed): cooking outputs, crop harvest, dungeon loot, companion procs. **Key insight — path is SHORTER than "model everything":** arm safely via a **server-owned-item-id predicate** that EXCLUDES the un-modelled set from the absolute overwrite (same shape as the existing `itemLedger.reconcile` carve-out), leaving them client-authored-but-safe. **Step 2 IN PROGRESS:** systems-engineer building that predicate + exclusions + guard test, UNARMED. Then: security review → restore drill → wipe → arm. Farming/dungeon server models = roadmap, NOT flip blockers. |
