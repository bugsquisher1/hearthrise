...
| **A floor that is too HIGH** (SEC S2, or a frame stamped wrong) | every frame refused, silently, for the whole session | ⚠ **NOTHING, YET.** `hello` heals a floor that is too *low*; a `hello` below a bad floor classifies `reorder` and is dropped like everything else, so the healer is gated by the thing it heals. What ships today is **detection**: `getFrameDrops()` counts consecutive refusals at all three appliers and `getAccrualState()` publishes `{frame, drops, verdict}` on the seam a bug report already carries. | The cure — after N consecutive drops, one forced full re-read that is allowed to **reset** the floor rather than be gated by it — is a **hard prerequisite of `lane/m5-live-subscribe`** (SEC §3), not of this lane. Until it lands, `tests/frame-drop-streak.mjs` D4 pins the hole itself and goes RED when it is closed, so the author who closes it must come and rewrite this row. |
