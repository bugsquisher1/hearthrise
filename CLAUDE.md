...
- **The browser never says one thing while the server says another (Tyler, 2026-09-14 01:30 UTC: "be sure there are no other instances that the browser says one thing and the server says another").** Every number a player can act on — an item count, a key, a seed, a level, a cooldown, a buff clock, a plot state, a bank stack, a gate — is rendered from the server's projection and REPLACED by each envelope, never merged upward, never predicted past the next settle without being reconciled. A client-side prediction may show for instant feedback only if the surface it feeds cannot spend it (no gate, no intent, no purchase reads it) and the next envelope overwrites it. Found three times on 2026-09-13 (Goblin Seal "have 2 / no key", phantom seeds, the Depot's dormant fold): the sweep is the standing guard, and any new "client shows X, server refuses" report is a P1 class-kill, not a bug.
...
- **Mobile:** media query `@media (max-width: 540px), (max-height: 540px) and (max-width: 1024px)` (measured 2026-09-07: the 1024 form appears 25×, the old 900 form 8× — converging those eight is cleanup slice 5; `tests/breakpoint-guard.mjs` freezes the set). Phones are landscape-only; a landscape phone gets the scaled-desktop rail layout, never the bottom-nav (b310). `.prof-toolbar` stays hidden on mobile; `.feat-buttons` is its equivalent.
