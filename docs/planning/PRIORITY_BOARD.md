...
| Item | Status | Pri | Notes |
|---|---|---|---|
| `legacy.js` render-layer split → `src/render/*` | 🔧 **active** | **P1** | Monolith 18.8k lines. Strangler-fig, one domain at a time, behind smoke + visual gate. Pattern: `docs/design/render-extraction-pattern.md`. **Done so far (~320 lines out of legacy.js):** #1 **b383** Lifetime Stats modal (fully visual-gate verified, identical); #2 **b384** Active Effects panel (content-verified + E2E + combat/home regressions pixel-clean, but the card's own pixels couldn't be screenshotted headless — **needs a 5-sec glance: open Profile, check the Active Effects card**). **Paused pending #2 glance** before stacking more UI changes. #3 candidate flagged: Companions/Stable double-render coupling (`legacy.js` block 32 vs `src/features/companions.js`) must be resolved before picking Stable. |
