...
// modularised, will import { G } from '../state/game.js?v=226' directly.
//
// Triggered by:
...
import { on, snapshot } from '../net/events.js?v=226';
import { findUiOverlaps, watchUiOverlaps } from './ui-overlap.js?v=226';
// b225: the save-conflict rule, lifted out of pullAndMaybeRestore() precisely
// so the "a local save is never discarded silently" promise is provable.
// b226: same reasoning for the auth-event rule — the cached session is what the
// account wall opens on, so "when may we delete it" has to be provable.
import { decideRestore, decideSessionEvent } from '../net/auth.js?v=226';
...
const TESTS = [
  ...
  () => tryRun('b229: a hidden, unfocused, untouched tab is STILL blessed — the tab need not be open', () => {
    ...
    try {
      ...
      assert(note.indexOf('in the game') >= 0, 'the note must state the real rule, got: ' + note);
    } finally {
      E._force(null);
  ...
  () => tryRun('b227: the live blessing note names only a blessing that touches THIS activity', () => {
    ...
    try {
      ...
      NS.setMode('ok');
      if (hit) {
        assert(live.indexOf(hit.name) >= 0 && live.indexOf("alive while you're in the game") >= 0,
  ...
  () => tryRun('b229: every surface states the same rule — in the game, not in this tab', () => {
    ...
    try {
      ...
      assert(cardTxt.indexOf('in the game') >= 0 && cardTxt.indexOf('steady base rate') >= 0,
      ...
      if (homeTxt.indexOf('The realm') >= 0) {
        assert(homeTxt.indexOf('in the game') >= 0,
          'Home\'s "The realm" must state the same rule, got a panel without it');
      }

      // 3 — the live activity note.
      const note = window.HearthriseBlessingNote();
      assert(note.indexOf('in the game') >= 0, 'the activity note must state the same rule, got: ' + note);
      ...
      assert(toast.indexOf('steady base rate') >= 0,
        'the offline toast must name the steady base rate, got: ' + toast);
