...
/* ── POST-CHAIN PLANTING (`post`) ─────────────────────────────────────────
   ...
   actually measures something:
     · gate mode       — `find`/`repl`, refused by this migration's own §7
                         gate at apply. That arm demonstrates the MIGRATION.
...
const MUTATIONS = {
  ...
  to authenticated;`,
    post: `grant execute on function public.hr_buy_gem_unlock__ungated(text, int, uuid)
  to authenticated;`,
  },
...
async function boot(mutate, gateBlind, extra) {
  ...
  /* The gate-blind half of a grant/ACL arm: the chain applies VERBATIM and the
     defect lands after it. No text patch, so no §7 to blind either. */
