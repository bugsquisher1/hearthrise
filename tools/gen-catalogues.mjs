...
/* THE FOURTH GENERATED FILE (2026-09-14, the gem-unlock projection lane).
   hr_gem_unlocks is the PRICE half of the theme/cosmetic unlocks: hr_unlocks
   already catalogues `theme:*` and `cosmetic:*` as flag STORAGE, but nothing in
   the database has ever known what one COSTS, which is why the only gem
   purchase path in the game was a client-side `G.gems -= price` (the b371 dupe)
   and why src/render/shop.js still passes the price into buyCosmetic(id, price)
   as an argument. The numbers come from src/data/shops.js, which tools/
   gen-shops.mjs already derives from the authored literals (HOUSE_THEMES in
   src/legacy.js, `const cosmetics=[` in src/render/shop.js), so this is the
   third link of ONE chain and not a second authoring site. Same operational
   reason as the two above for being its own LAST-registered file. Covered by
   the same `--check`. */
const OUT_GEM_UNLOCKS = join(ROOT, 'supabase', 'migrations',
...
// ── 4c. Emit the GEM-UNLOCK PRICE catalogue (its own migration file) ─────
/* WHAT IS AND IS NOT IN HERE, decided by a PREDICATE rather than a list: an
   offer qualifies when its table is `theme` or `cosmetic`, its cost is EXACTLY
   one currency line, that currency is `gems`, and the amount is > 0. Everything
   that predicate excludes is excluded for a reason the reader can check:
     · theme.default costs `gold 0` — the free equip (src/legacy.js buyTheme's
       slice-6 note). A zero-priced offer in a purchase catalogue is an infinite
       faucet, so it must not be sellable, and "not in the table" is the only
       form of that rule nobody can forget.
     · every other gem sink (bank.gem, character_slot.*, the IAP packs) belongs
       to a verb that already owns its own catalogue — hr_hero_slots, BANK_SPACE,
       platform billing. Two catalogues for one price is the drift this repo
       generates its SQL to avoid.
   The grant is read from the offer too, never rebuilt from the id: `theme.forest`
   grants `theme:forest`, and it is the GRANT that must match hr_unlocks, because
   that is the row public.player_progress will actually carry. */
const { SHOP_OFFERS } = await imp('src/data/shops.js');
const GEM_UNLOCK_TABLES = ['theme', 'cosmetic'];
const gemUnlocks = SHOP_OFFERS
  .filter((o) => GEM_UNLOCK_TABLES.includes(o.table))
  .map((o) => ({
    o,
    cost: Array.isArray(o.cost) ? o.cost : [],
    unlocks: (Array.isArray(o.grant) ? o.grant : []).filter((g) => g.kind === 'unlock'),
  }))
  .filter(({ cost }) => cost.length === 1 && cost[0].kind === 'currency'
    && (cost[0].id === 'gems' || Number(cost[0].amount) === 0))
  .map(({ o, cost, unlocks }) => {
    if (unlocks.length !== 1) {
      console.error(`gem-unlocks: ${o.id} grants ${unlocks.length} unlocks, need exactly 1`);
      process.exit(1);
    }
    const unlockId = String(unlocks[0].id);
    if (!unlockId.startsWith(`${o.table}:`)) {
      console.error(`gem-unlocks: ${o.id} (table ${o.table}) grants "${unlockId}" — the namespace `
        + 'must be the table, or the flag row lands somewhere hr_unlocks does not catalogue');
      process.exit(1);
    }
    if (Number(unlocks[0].amount) !== 1) {
      console.error(`gem-unlocks: ${o.id} grants amount ${unlocks[0].amount} — a flag is 1`);
      process.exit(1);
    }
    const amount = Number(cost[0].amount);
    if (!Number.isInteger(amount) || amount < 0) {
      console.error(`gem-unlocks: ${o.id} costs ${cost[0].amount} — not a whole non-negative number`);
      process.exit(1);
    }
    return {
      unlock_id: unlockId,
  ...
  .sort((a, b) => (a.unlock_id < b.unlock_id ? -1 : 1));

if (!gemUnlocks.some((r) => !r.free)) {
  console.error('gem-unlocks: the predicate matched no PRICED offer — src/data/shops.js has changed '
    + 'shape and this catalogue would ship with nothing for sale, which reads as a design decision '
    + 'rather than as a broken generator');
  process.exit(1);
}
const GEM_UNLOCK_NS = [...new Set(gemUnlocks.map((r) => r.namespace))].sort();
const GEM_UNLOCK_DIGEST = createHash('sha256').update(JSON.stringify(gemUnlocks)).digest('hex');
...
--   tests/run-sql-tests.mjs and tests/run-smoke.mjs.
--
--   catalogue digest: ${GEM_UNLOCK_DIGEST}
--   rows: ${gemUnlocks.length} (${gemUnlocks.filter((r) => !r.free).length} priced, ${gemUnlocks.filter((r) => r.free).length} free) across ${GEM_UNLOCK_NS.length} namespaces (${GEM_UNLOCK_NS.join(', ')})
--
...
${valuesBlock(gemUnlocks, (r) => `  (${q(r.unlock_id)},${q(r.namespace)},${q(r.offer_id)},${q(r.name)},${n(r.cost_gems)},${b(r.free)})`)};
...
begin
  ...
  if v_n <> ${gemUnlocks.length} then
    raise exception 'hr_gem_unlocks has % rows, generator emitted ${gemUnlocks.length}', v_n;
  end if;
  ...
  raise notice 'hr_gem_unlocks OK — % rows, digest ${GEM_UNLOCK_DIGEST}', v_n;
end $$;
`;
...
if (CHECK) {
  ...
  // THE FOURTH FILE, same rigour, reported SEPARATELY — a `--check` that went
  // green on a stale hr_gem_unlocks would let the shop card's price and the gems
  // hr_buy_gem_unlock actually debits drift apart, which on a money surface is
  // the one drift nobody may discover from a support ticket.
  const existingGemUnlocks = await readFile(OUT_GEM_UNLOCKS, 'utf8').catch(() => null);
  if (existingGemUnlocks === null) {
    console.error(`gem-unlock catalogue drift: ${OUT_GEM_UNLOCKS} is missing. `
      + 'Run: node tools/gen-catalogues.mjs');
    process.exit(1);
  }
