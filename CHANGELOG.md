...
**The realm now scores your Renown record and enforces dungeon cooldowns.** Two server-side changes landed today: your Renown high-water mark is kept by the realm and shown at the top of Home and Character, and dungeon runs honour a per-mode cooldown the realm reports (the door you saw in build 541 now has a real clock behind it).

**That Renown sentence is gone.** "Your best yet. New gains count from your next settle" read like a riddle; the headline is now just your rank and number, with a short tooltip on the figure.

**Under the hood:** dead slot-switch helpers and a settle hook that never ran were removed; the ledger test learned the realm’s one-row-per-Renown-raise contract; the economy fuzz plants its gold bug where production actually runs it.
...
**A refused tap always answers.** Tapping a fishing spot while knocked out used to do nothing at all; it now shows the sheet, or a notice if the sheet cannot draw.
...
- 🧭 **The tour tells the truth.** Two of its lines taught rules that no longer exist ("nobody does it for you yet", "a fight ends when you fall"); they now say what happens — a fall knocks you out and your run picks up, Auto-Eat keeps you fed — and the Bounty Board gets named.
- ☁️ **Settings › "Verify cloud save" reads the realm, not a retired table.** It reports the version the realm holds, when it last settled, gold and total level; a failure says it is a connection problem, not lost progress.
- 🪨 **Your bench run survives a reload.** Log out mid-quarry (or mid-cook, mid-smelt) and the realm kept running and paying the bench — but the screen came back saying "Idle", because the client's boot path could resume a fight or a gathering run and had never learned to resume a bench. It resumes now, and a run the realm already owns is never re-declared on wake. (Reported by Paione.)
- 🛌 **Knocked out and reloaded? The game knows.** After a reload while recovering, the client did not learn the recovery clock until the first settle, so the countdown and the knocked-out banner were missing and a start attempt could slip through. Recovery is now hydrated at boot like hit points are.
- 🏠 **The Home away card keeps the night's story after a reload.** The realm now stores your last away receipt and hands it back on load, so "you were away 9h — here's what happened" survives a refresh instead of being overwritten by the first ordinary sync.
- 📖 **No more "New discovery" for things you have held for days.** The collection log only learned about items the client itself picked up; everything the realm granted while you were away (or during a settle) never reached it, so the first attended tick that touched a long-held stack announced it as new. The log now records every item the realm's bag holds, silently, before deciding anything is a discovery. (Reported by Paione, 14.8K granite.) The same blind spot exists for the bestiary's first-kill badges and is queued.
- 🦴 **Burying bones is a real bench now, not a button that lies.** The instant Bury action removed the bones and added Prayer XP on your screen only; the realm never saw it, so a reload gave the bones back and took the XP away. Bury now starts the altar bench the realm settles (4.5 Prayer XP a bone, bones consumed for real), and a new guard counts every place the client could hand itself XP — that count is now zero and may only fall. Prayer no longer needs the Shrine to be built — the Shrine makes burying faster, it was never meant to make it possible, and the realm has only ever asked for the level. (Reported by Paione, twice — the first report was dropped, and that is on us.)
- ⚔️ **Opening your bag mid-fight no longer disarms you.** Any inventory or loadout repaint made the equipment record believe the client had overwritten the realm's worn set, and the record fail-closed to "naked": your swings went from a 2H hammer's 3.17 s to 2.4 s, the strength bonus and the weakness multiplier vanished, and max hit fell from 30 to 25 — real lost damage, not a display glitch, until the next equip. An empty slot is no longer counted as something you wore. (Reported by Paione, with the clip that showed the exact numbers.)
- ⚒️ **Smithing and Crafting no longer wait for the Forge or the Workshop.** Levels grant permission; rooms grant speed and quality — the rule Cooking and Prayer already lived by. A new miner can smelt the first copper bar at the camp instead of carrying ore through two property upgrades; the rooms keep their speed bonuses and will pick up their own reward (the odd extra bar, the piece that costs nothing) in a follow-up. The old mechanism that let skill XP quietly "grant" a room in local state is gone for good.
- 🧱 Under the hood: the icon layer (1,011 lines) left the 21,935-line monolith for `src/render/icons.js` with a zero-difference proof across 2,835 helper calls, and it found a real bug on the way — three item glyphs (iron fitting, timber beam, muster seal) had been drawing through a bypassed wrapper. Four debt ratchets now hold the line: the monolith may only shrink, render modules may only grow, comment ratios and migration patch-chains may only fall, the test file may only get denser.
...
- 🐾 **Companion level-ups show again.** Since companion XP moved to the realm, the level arrived silently and the pet cards never repainted (the old celebration lived in a path nothing could reach). The client now notices a level change in the realm's answer, repaints the cards once, and names the new level.
- 🏠 **Your away card no longer vanishes a minute into play.** The Home card that sums up the night was overwritten by the first ordinary sync receipt (which has nothing to say), so the story of your absence disappeared about ninety seconds after you sat down. The away receipt now has its own holder: a sync never evicts it, and the card stays for the full thirty minutes it was always meant to.
- 🎨 **One source for the game's colours and spacing.** 367 design-token declarations were scattered across four stylesheets, 41 of them defining the same token twice with different values and the loser silently ignored. They now live in one file loaded first. Nothing on screen changed — verified pixel-for-pixel on eight screens at desktop and phone sizes — and a guard keeps it that way.
- 📜 A process rule from an incident today: no `git stash` in any agent worktree — the stash is repo-wide and a pop in one lane took another lane's uncommitted work (recovered intact).
- 🧪 The one red on b518's first parallel CI run is explained and fixed: the database-growth guard's second sample read a planner statistic that is zero right after a cached-chain restore, so the guard fired on a cache miss and went silent on a cache hit — and the new CI caches the chain per job. It now analyzes before sampling, and a sample it cannot take is a loud harness error, never "0 alerts". A separate memory fault (WebAssembly could not allocate) was reading as a normal red; it's classified as harness now.
...
- ⚔️ **Combat credit stops the moment the realm says you are not fighting.** The kill and XP cadence now refuses (instead of paying) any window that falls inside a knockout or after your activity left combat; the realm stamps the switch moment itself so an auto-stop can never strand honest credit; a recovery clock can never be shortened by a later write. If the realm's clock and yours disagree by a hair at the boundary, the client re-declares once and retries — never loops.
- 🧹 Behind the scenes: eleven "dormant" flags whose comments lied about being off now tell the truth (with a guard); the three raw gold writes left in the client refuse when the record isn't writable; cache-busting fixed for swapped icons; a guard pins the eight same-named functions that live in separate modules so a real shadowing bug is loud.
- 🍤 **Your bag no longer shows food you already ate.** Cooked food eaten by an away fight could linger on your screen forever (a server count of zero simply stopped being mentioned, and the client never lowered a cooked stack). The knocked-out sheet then told you "you were carrying 20 Cooked Shrimp and never ate one" and "Rest at the Hearth" failed silently. Eaten food now reads true on every complete sync, the sheet prices Rest from the real bag ("No food to rest with" when it's empty), and a refused Rest keeps the sheet open and says why — nothing was eaten.
- 🗝️ **Dungeon rewards are real again.** Since the cutover, dungeon scrip and loot were minted on your screen only and vanished on the next reload — no dungeon had ever been settled by the realm. The realm's settlement (already built and reviewed) is now switched on: scrip, loot and the key debit come from the server, survive reload, and a retried key can't double-spend.
- 🌾 **Farm plot tiers are reachable.** Since the wipe every farm was tier 1 and the only way up was a Farmer's Deed that almost never drops, so nobody could plant past turnips. Plot upgrades are now priced in **gold** — 500 for tier 2 (an evening of turnips), then 5,000 / 25,000 / 100,000 — with a farming-level gate per tier (5 / 25 / 45 / 70) and the old deed counts (1 / 3 / 5 / 8) kept as an alternative payment, so deeds become a shortcut and a market good instead of a wall. The realm prices and charges the upgrade itself; House → Plot quotes the real price and what you have.

## v0.9.2-beta build 510 — 2026-09-06 (A fall at the keyboard counts too)

- ⚔️ **Dying while you sit and fight now goes through the same Recovery Rule as an away death.** b509's promise only held for away runs: at the keyboard, a death quietly ended the fight, the realm was never asked, and you stood back up at full health for free — the exact loophole the rule exists to close. Now a fall stops only your swing; the fight stays declared, the realm prices it with the one combat engine, records the fall, sets the recovery clock and hands you back at 40% health when it ends. The knocked-out sheet reads the realm's answer only — no more client countdown that could quote a fifth-fall timer on your first fall.
- 🔕 **"Synced — +N items" no longer interrupts you every ninety seconds while you play.** (Reported by Paione.) A live settle you watched happen narrates nothing; a real absence, a switch, a death and a market sale still get their receipts — attendance has to be proven, never assumed, so a fresh boot after closing the tab still speaks.
- 🎁 **Quest item rewards are real again.** Since the cutover every quest's *item* reward existed only on your screen and vanished on the next reload (the realm credited gold alone). Rewards now live in a server-side catalogue and are credited into your bag in the same transaction as the gold, once per character, journalled. "First Cook" pays its 200 gold **and 30 shrimp** again. Characters who already claimed it before today were paid gold only — tracked.
- 🌱 **Planting tells you the truth.** (Reported by Paione.) When the realm refuses a plant it now says exactly why and what clears it — "Carrot needs Farm Plot Lv 2 — upgrade in House → Plot (you have Lv 1)", no seed, plot cap, level — instead of a seed that appears and silently vanishes. The seed picker shows locked crops with the tier they need, House → Plot prices from the realm's own tier, and the plant gate reads that tier too, so the game never offers what the realm will refuse. (Behind this: since the wipe every farm is tier 1 and nobody could see the refusals, so planting beyond turnips had quietly stopped for everyone. The tier ladder itself is being re-priced so tier 2 is reachable in an evening — next build.)

## v0.9.2-beta build 509 — 2026-09-06 (Falling no longer ends your night)

- 🌙 **Dying while you're away no longer wastes the rest of the night.** Until now an away fight ended at your first death and everything after it paid nothing — for a new character without food that meant a twelve-hour night worth about a minute. Now a fall knocks you out for a recovery period, and then you get back up **at 40% health and pick up the same activity**. The first fall of each day costs nothing; each further fall the same day doubles the recovery (2m, 4m, 8m… up to 64m), so it can never be farmed but it never abandons you either. Recovery applies to *everything* you could be doing — fighting, gathering, crafting — and it is clocked by the realm, so a reload cannot skip it.
- 🍖 **Rest at the Hearth.** Knocked out and impatient? Eat your way back: the death screen offers to consume enough provisions to cover your missing health and clears the timer on the spot. Food is the fastest way off the floor — by design, it costs exactly what an unbroken fed night would have.
- 🧾 **The welcome-back card tells the whole story honestly** — what you gained, that your run picked up after every fall, what a stocked bag would have been worth (capped at "more than 25×" so the number never reads as a boast), and your falls on one line, with the reason nothing healed you named plainly ("You had no cooked food…").
- 🍳 **Auto-Eat is switched on for everyone who already owns it.** Almost every account owned Auto-Eat I but had it off — the realm granted the perk, not the switch. On your next load the game turns it on once and tells you, with a one-tap **Keep it off** if you'd rather not; whatever you choose is remembered.
- 🏰 Hearth Hall Premium's description no longer promises perks that don't exist yet ("exclusive cosmetics, monthly chests"). Nothing is sold before it is built.
- 🏠 **The Home away card tells the same true story** — on a night with several falls it now says you fell, how long you were out in total, and that your run picked up each time, instead of claiming nothing was earned after the first death. (Two related receipt bugs caught by the gate before release: the welcome-back card had lost its death line for the older receipt shape, and a recovery clock could leak between sessions in the test harness.)
- 🧪 Behind the scenes: every fall is journalled on the realm's ledger; a standing guard fails the build if any shop, trait, ladder or bond ever sells recovery relief; the `first_cook` quest reward is gold-only again because quest *item* rewards don't yet persist server-side (tracked); and the security review's attended-credit follow-up is scheduled for the next build.

## v0.9.2-beta build 508 — 2026-09-06 (Your tap sticks, your bank stays bought, your farm remembers)

- 🔁 **Switching activities now actually switches.** If the realm couldn't price your current activity's window at the exact moment you tapped a new one, the tap was quietly abandoned and your screen snapped back to the old activity a few seconds later with no message — in an idle game, that's a whole session spent on the wrong thing. The game now settles the old window and re-asks once for the activity you chose; if it still can't, it tells you plainly ("Couldn't switch — still fighting Forest Goblin. Tap again to retry.") instead of pretending.

- 🏦 **Bank space you bought stays bought.** Reloading snapped your bank back to the 100-slot base, started the "Bank full" nags, and made you press *Buy* once per rung you already owned just to be told "that space is already yours." Two faults: a duplicated default in the game's starting state wiped the purchase counters, and nothing read your purchases back from the realm. Your owned rungs are now restored from the realm's own records on every load. *(Open since SA-010 — thank you for the reports.)*
- 🌱 **Your farm remembers its upgrades and its watering.** Reloading dropped a Farmer's-Deed-upgraded plot back to level 1 (locking the seeds you'd unlocked, and quoting the wrong price for the next upgrade), and collapsed a crop you'd watered several times down to a single watering — so a crop the realm considered ready showed a long timer and a *Water* button instead of *Harvest*. The realm now sends both facts with every load, and the client mirrors them exactly (it computes nothing of its own; harvest and upgrade pricing were always decided server-side).

- ❤️ **Your max health now follows your Hitpoints level — for everyone, retroactively.** Every character's ceiling had been pinned at 10 since creation: levelling Hitpoints raised the number on the skills screen but never the bar you fight with, which also dragged Auto-Eat's trigger down and made every away fight harder than it should be. The realm now derives max HP from your Hitpoints XP whenever it changes, and a one-time correction lifted every existing character to the ceiling their XP had already earned (each correction is journalled). *(Found by playing: a level-12-Hitpoints character fighting at 10/10.)*

- 🧾 Behind the scenes: the repo's record of which database changes are live had drifted — **fourteen** migrations were marked "staged" that production had been running for days. Every note is corrected with hash evidence, and a new check fails the build if the record ever disagrees with what's measured live again. Also root-caused why the previous build's automated checks went red on one step (the removed store products changed what a rebuilt catalogue contains; the census is re-pinned) — the one class of check that only runs in CI now runs locally before every release too.
- 🏦 **…and the realm honours the bank space you bought, too.** The client fix above restores what you *see*; this closes the other half — the realm's own limit never rose when you bought a rung, so past 100 stacks it kept refusing item pickups regardless of what you'd paid. Purchased rungs now raise the enforced limit the instant they're recorded, and existing owners were lifted to the capacity they'd already bought.

## v0.9.2-beta build 507 — 2026-09-06 (An honest store, an honest receipt, and nothing lost at the door)

- 🛒 **Four store products are gone for good.** *Remove Ads* sold the removal of ads that never existed. *Lifetime Offline+* and Hearth Hall's *"+25% offline progress"* sold faster idle progress on a shared, ranked economy — the one thing this game promised never to sell (we deleted the Season Pass for the same reason). And the *Starter Bundle* sold **200,000 gold for cash**, which would have minted the tradeable currency straight from a credit card. All four are removed; Hearth Hall Premium stays, minus the offline boost.
- ⚖️ **A standing rule, enforced by the build:** no purchase may ever grant gold, and nothing you can buy may raise the rate or total of offline progress. The offline cap is a flat 12 hours for everyone. If anyone tries to add such a product, the release fails.
- 🧾 Behind the scenes: the client code path that could have credited purchased gold is deleted (not just unused), an old test that asserted the *opposite* rule was corrected, and the generated shop catalogue the realm authorises from was regenerated in step. *(The realm's own copy of the catalogue updates with this deploy; three now-dead offer rows remain inert in the database until a reviewed cleanup.)*

- ⚔️ **New characters now fight in *Controlled* style by default, training Attack, Strength AND Defence together.** The old default, *Accurate*, trained Attack alone — so anyone who never opened the style picker (most new players) ended up with Attack in the teens and Strength and Defence still at level 1 after hundreds of kills: tiny hits, slow kills, and a permanently throttled combat-XP cap. We found it by playing a fresh account. Your chosen style is untouched; only the "chose nothing" default changes, on your screen and in the realm's settle alike.
- 🧪 Behind the scenes: the regression test checks the XP *routing* itself (all three melee skills paid per hit and per kill), and the client/server default now comes from one shared table so the two can never disagree again.
- 🌙 **Your welcome-back card now reports only the level-ups the realm actually banked.** Since the live-settle split, the card listed every level crossed inside the realm's *simulation* of your absence — but only the not-yet-credited part of that window was written, so it could promise "Strength 1→4" while your Strength stayed 1. We reproduced it exactly (a 90%-credited window claimed 60 level-ups and banked 23). The receipt is now derived from the XP that was written, and a guard fails the build if the card ever again reports a level the write didn't produce.
- 📅 Behind the scenes: the daily "gain a skill level" goal now counts *banked* levels too (it used to credit levels that never persisted).

- 💾 **Closing the tab no longer loses your last minute.** The save that fires when you close or background the game was being cut off by the browser before it finished, so up to 60 seconds of progress that only your device tracks — new bestiary entries, achievements, quest progress, "already claimed" markers on daily rewards and dungeon cooldowns, your loadouts and combat style — could silently vanish. That parting save is now sent in the browser's keep-alive mode, the same way your away-progress settle already was. *(Found by the persistence sweep, not a player report — it was the sort of loss you'd never be sure you'd seen.)*
...
- 🗝️ **Combat keys are real drops now.** Bone keys, goblin seals and the other dungeon keys are granted and recorded by the realm as part of the monster's own drop table — the old client-side key handout that the realm never recorded (and that a reload therefore erased) is gone. They drop while you're **away** now too and show on your welcome-back card. *(Player report — thank you.)*
- ⚠️ **Straight talk on "items vanish after a fight" — measured live, next fix.** During an attended fight the realm currently credits roughly 60% of the drops you see, and food eaten mid-fight can come back, because the after-fight settle re-simulates your session too conservatively. Kills, XP and gold are credited correctly. This is the root of the remaining vanishing-item reports; the server-side crediting fix is the top item.
...
## v0.9.2-beta build 476 — 2026-08-25 (Your workers show up again)

- 👷 **Fixed hired workers vanishing from the House.** If you were idle while your crew was out gathering, the game could show an empty worker roster even though your workers existed and were producing on the server — so clicking "Hire" seemed to do nothing (a worker appeared, then disappeared, and no gold moved). Your crew now always shows, and hiring re-materialises any worker you already paid for without charging you again. (Server/edge fix — no new download needed for the roster; the hire button itself is in this build.)
- 🛠️ Hiring is now "ask the server first": it fills any worker slot you've already paid for **before** it ever charges you, so a slot you bought can never be stranded unpaid-for-nothing.
...
- ⏳ **Fixed the alarming "Away progress is paused" message** that could appear on login for players returning idle with banked rested XP or a worker crew. Your progress was never actually paused — the server had already granted it, but a missing piece of the reply made the game think the update was broken and hide it. Returns now show your away progress correctly. (Server/edge fix — no new download needed.)
