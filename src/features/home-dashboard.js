...
(function () {
  ...
  function awayCardHtml(off) {
    ...
    /* b345: a stop that paid NOTHING is as quiet as a declined night, and the
       card must react the same way — say "your camp was quiet" instead of
       leaving a bare duration, drop the base-rate line (noise where an
       explanation belongs), and offer the CTA. Keyed on the stated stop plus
       an empty ledger, never on the empty ledger alone: a legitimately
       zero-yield night is not a stop. */
    var quiet = !!(lic || stop) && !bits.length && !combatBits.length;
    if (lic) {
