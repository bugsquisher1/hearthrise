...
_Last updated: 2026-08-30 (b497 assembly in flight). **🔒 GOLD+GEMS ARMED — server-authority program COMPLETE.**_
...
| **Enable PITR** (paid add-on) | recommend YES | Census measured 2026-08-30: PITR OFF = up to **24h of every player's progression unrecoverable** on one bad migration; 20,832 restore-only rows. Paid → blocked on budget freeze; needs your explicit go. Takes the window from ~24h to minutes. |
...
| `hr_unlocks` seed divergence: rebuild=82, prod=65 | 🧊 | P2 | Restore-census live-compare (2026-08-30): the whole delta is `namespace='companion'` (4 vs 21) — a rebuild ships 17 companion unlocks prod never got. Backend to rule: apply the missing seed to prod, or fix the repo seed. Census reports it every run until closed. |
